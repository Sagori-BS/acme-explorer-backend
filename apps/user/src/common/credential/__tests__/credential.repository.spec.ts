  ...
  describe(`update${entityName}Password`, () => {
    it(`should update the ${entityName} password given a valid input`, async () => {
      ...
      const updateCredentialPasswordInput: UpdateCredentialPasswordInput = {
        ...
        data: {
          password: createEntityInput.password + '2',
        },
      };
      ...
      const passwordsMatch = await bcrypt.compare(
        updateCredentialPasswordInput.data.password,
        result.password,
      );
    ...
    describe('provided oldPassword', () => {
      it(`should throw PasswordMismatchError if oldPassword does not match current password`, async () => {
        const entity = await createDocument();
        ...
        const updateCredentialPasswordInput: UpdateCredentialPasswordInput = {
          ...
          data: {
            password: createEntityInput.password + '2',
            oldPassword: createEntityInput.password + '3',
          },
        };

        const result = entityRepository.updateCredentialPassword(
          updateCredentialPasswordInput,
        );

        expect(result).rejects.toThrow(PasswordMismatchError);
      });

      it(`should update the  ${entityName} password if oldPassword match current password`, async () => {
        const entity = await createDocument();
        ...
        const updateCredentialPasswordInput: UpdateCredentialPasswordInput = {
          ...
          data: {
            password: createEntityInput.password + '2',
            oldPassword: createEntityInput.password,
          },
        };
